...
gauss_conv_line_3d_yz_script= torch.jit.script(gauss_conv_line_3d_yz, example_inputs=[
                                               ...
                                               torch.rand(3, 2, 1, 1, 3, 1, 1),
                                               torch.rand(3, 1, 2, 1, 1, 3, 1),
                                               torch.rand(3, 1, 1, 2, 1, 1, 3)])
...
def test_consistency(Q, X0, X1, Sigma, x, y, z, mask, device='cuda'):
    ...
    result2 = gauss_conv_line_3d_yz_script(Q, X0, X1, Sigma, x, y, z)
    ...
    result1_mask = result1
    result2_mask = result2
    ...
    for ff in ffs:
        ...
        with torch.no_grad():
            tstats = benchmark.Timer(
                stmt = 'gauss_conv_line_3d_yz(Q, X0, X1, Sigma, x, y, z)',
        ...
        # with torch.no_grad():
        #     test_consistency(Q, X0, X1, Sigma, x, y, z)

        # del charge
        torch.cuda.empty_cache()
        torch.cuda.synchronize()
        q1 = gauss_conv_line_3d_orig(Q, X0, X1, Sigma, x, y, z, device)
        q2 = gauss_conv_line_3d_yz_script(Q, X0, X1, Sigma, x1d, y1d, z1d)
        assert q1.allclose(q2, atol=1E-5, rtol=1E-5)


    # print(torig, tmask)
    return ffs, tyz, tyzscript, torig, tmask
...
if __name__ == "__main__":
    ...
    ffs, tyz, tyzscript, torig, tmask = main()
    torig = np.array(torig).mean(axis=0) * 1E3
    tyz = np.array(tyz).mean(axis=0) * 1E3
    tyzscript= np.array(tyzscript).mean(axis=0) * 1E3
    ...
    plt.hlines(tyz, xmin=ffs[0], xmax=ffs[-1], linestyles='dashdot', label='w/o masks; x,y,z,broadcast;optimized')
    plt.hlines(tyzscript, xmin=ffs[0], xmax=ffs[-1], linestyles='dashdot', color='r', label='w/o masks; x,y,z,broadcast;optimized;jit.script')
    ...
    plt.savefig('profile_masks_claude8.png')
