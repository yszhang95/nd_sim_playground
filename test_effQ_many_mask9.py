...
def gauss_conv_line_3d_yz(Q, X0, X1, Sigma, x, y, z):
    """
    ...
    result_shape = [batch_size] + (len(x.shape)-1) * [1]
...
gauss_conv_line_3d_yz_script= torch.jit.script(gauss_conv_line_3d_yz, example_inputs=[
                                               torch.rand(3), torch.rand(3,3), torch.rand(3,3), torch.rand(3,3),
                                               torch.rand(3, 3, 1, 1),
                                               torch.rand(3, 1, 3, 1),
                                               torch.rand(3, 1, 1, 3)])
...
def gauss_conv_line_3d_mask_optimized(Q, X0, X1, Sigma, x, y, z, mask, device='cuda'):
    ...
    charge = torch.zeros(result_shape, device=device, requires_grad=False, dtype=torch.float32)
    # if len(b_indices) == 0:
    #     return charge
    ...
    # return charge
    return torch.cat([Q_masked, deltaSquareSqrt, deltaSquareSqrt, deltaSquare, sxsy2, sysz2, sxsz2, sx2, sy2, sz2, dx01, dy01, dz01])
...
# def gauss_conv_line_3d_mask_optimized2(Q, X0, X1, Sigma, x, y, z, mask):
#     result_shape = x.size()
#     charge = torch.zeros(result_shape, dtype=torch.float32, requires_grad=False, device=Q.device)
#     batch_idx, x_idx, y_idx, z_idx = torch.where(mask)
#     with torch.no_grad():
#         charge_vals = gauss_conv_line_3d_mask_optimized2_kernel(Q, X0, X1, Sigma, x, y, z, mask)
#     # charge[batch_idx, x_idx, y_idx, z_idx] = charge_vals
#     return charge

# @torch.jit.trace
# def gauss_conv_line_3d_mask_optimized2_kernel(Q, X0, X1, Sigma, x, y, z, mask):
# @torch.jit.script
# def gauss_conv_line_3d_mask_optimized2(Q, X0, X1, Sigma, x, y, z, mask):
#     """
...
#     X0T = X0[batch_idx].T
#     X1T = X1[batch_idx].T
#     SigmaT= Sigma[batch_idx].T
#     x0, y0, z0 = X0T[0], X0T[1], X0T[2] # Transpose to get separate coordinates
#     x1, y1, z1 = X1T[0], X1T[1], X1T[2] # Transpose to get separate coordinates
#     sx, sy, sz = SigmaT[0], SigmaT[1], SigmaT[2]
#
...
#
#     # Calculate denominators [batch_size, 1]
#     QoverDeltaSquareSqrt4pi = Q_batch / (deltaSquareSqrt * 4.0 * np.pi)
#     erfArgDenominator = sqrt2 * deltaSquareSqrt * sx * sy * sz
#
#     # Calculate final values efficiently
#     charge_vals = ((-QoverDeltaSquareSqrt4pi * torch.exp(
...
@torch.jit.script
def gauss_conv_line_3d_mask_optimized2(
    Q: torch.Tensor,
    X0: torch.Tensor,
    X1: torch.Tensor,
    Sigma: torch.Tensor,
    ...
    """
    Optimized version with minimal memory allocations and better vectorization
    """
    sqrt2 = 1.4142135623730951
    ...
    # Get parameters for the relevant indices - use view for better memory layout
    Q_batch = Q[batch_idx]
    X0_batch = X0[batch_idx].view(-1, 3)
    X1_batch = X1[batch_idx].view(-1, 3)
    Sigma_batch = Sigma[batch_idx].view(-1, 3)

    # Extract coordinates - avoid transpose for better memory layout
    x0, y0, z0 = X0_batch[:, 0], X0_batch[:, 1], X0_batch[:, 2]
    x1, y1, z1 = X1_batch[:, 0], X1_batch[:, 1], X1_batch[:, 2]
    sx, sy, sz = Sigma_batch[:, 0], Sigma_batch[:, 1], Sigma_batch[:, 2]

    # Pre-compute squared terms in-place
    sx2 = sx.square()
    sy2 = sy.square()
    sz2 = sz.square()
    sxsy2 = (sx * sy).square()
    ...
    # Calculate delta square and sqrt in one go
    deltaSquare = (
        sysz2 * dx01.square() +
        sxsy2 * dz01.square() +
        sxsz2 * dy01.square()
    ...
    # Calculate denominator for erf
    erfArgDenominator = sqrt2 * deltaSquareSqrt * sx * sy * sz

    # Pre-compute common terms for exponentials
    # exp1_arg = -sy2 * (xpos * dz01 + (z1*x0 - z0*x1) - zpos * dx01).square() * 0.5/deltaSquare
    # exp2_arg = -sx2 * (ypos * dz01 + (z1*y0 - z0*y1) - zpos * dy01).square() * 0.5/deltaSquare
    # exp3_arg = -sz2 * (ypos * dx01 + (x1*y0 - x0*y1) - xpos * dy01).square() * 0.5/deltaSquare

    # # Compute exponential product
    # exp_product = torch.exp(exp1_arg + exp2_arg + exp3_arg)
    ...
    # # Final calculation
    # charge_vals = (-Q_batch / (deltaSquareSqrt * 4.0 * np.pi)) * exp_product * (
    #     torch.erf(erf_arg1) - torch.erf(erf_arg2)
    # )

    return torch.cat([Q_batch, deltaSquareSqrt, deltaSquareSqrt, deltaSquare, sxsy2, sysz2, sxsz2, sx2, sy2, sz2, dx01, dy01, dz01])

# gauss_conv_line_3d_mask_optimized2 = torch.jit.trace(gauss_conv_line_3d_mask_optimized2_kernel, example_inputs=[
#                                                torch.rand(3), torch.rand(3,3), torch.rand(3,3), torch.rand(3,3),
#                                                torch.rand(3, 3, 3, 3),
#                                                torch.rand(3, 3, 3, 3),
#                                                torch.rand(3, 3, 3, 3), torch.rand(3, 3,3,3)])
...
def main():
    ...
    # q = gauss_conv_line_3d_yz_script(Q, X0, X1, Sigma, x1d, y1d, z1d)
    q = torch.zeros((nevent, ndim, ndim, ndim))
    ...
    for ff in ffs:
        ...
        # charge = torch.zeros_like(q, requires_grad=False, device=device)
        for i in range(10):
            gauss_conv_line_3d_mask_optimized2(Q, X0, X1, Sigma, x, y, z, mask)
        with torch.no_grad():
            tstats = benchmark.Timer(
                stmt = 'gauss_conv_line_3d_mask_optimized2(Q, X0, X1, Sigma, x, y, z, mask)',
        ...
        # with torch.no_grad():
        #     tstats = benchmark.Timer(
        #         stmt = 'gauss_conv_line_3d_yz_script(Q, X0, X1, Sigma, x, y, z)',
        #         setup = 'from __main__ import gauss_conv_line_3d_yz_script',
        ...
        # torch.cuda.empty_cache()
        # torch.cuda.synchronize()
        q1 = gauss_conv_line_3d_mask_optimized(Q, X0, X1, Sigma, x, y, z, mask, device)
        q2 = gauss_conv_line_3d_mask_optimized2(Q, X0, X1, Sigma, x, y, z, mask)
        assert q1.allclose(q2, rtol=1E-5, atol=1E-5)
    print(
            # gauss_conv_line_3d_mask_optimized2.graph_for(Q, X0, X1, Sigma, x, y, z, mask)
            # gauss_conv_line_3d_yz_script.graph_for(Q, X0, X1, Sigma, x1d, y1d, z1d)
        )

    # print(gauss_conv_line_3d_yz_script.code)
    # print(gauss_conv_line_3d_mask_optimized2.code)
...
if __name__ == "__main__":
    ...
    plt.legend(title='coef. only; no exp/erf')
    plt.title(f'output shape {tuple(shape)}')
    plt.savefig('profile_masks_opt2_v9.png')
    plt.yscale('log')
    plt.savefig('profile_masks_opt2_log_v9.png')
