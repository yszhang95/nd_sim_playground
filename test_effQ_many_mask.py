...
def gauss_conv_line_3d_mask(Q, X0, X1, Sigma, x, y, z, mask_indices, device='cuda'):
    """
    Calculate the 3D Gaussian convolution along a line segment with per-sample masking.
    This function computes the charge distribution from a line-segment source
    convoluted with a 3D Gaussian distribution, but only at the specified indices.
    ...
    Args:
        ...
        mask_indices (torch.Tensor): Indices where to compute, shape: [num_points, 4] where
            each row is [batch_idx, x_idx, y_idx, z_idx]
    ...
    x_indices = mask_indices[:, 1]
    y_indices = mask_indices[:, 2]
    z_indices = mask_indices[:, 3]
    ...
    charge[batch_indices, x_indices, y_indices, z_indices] = masked_charge
    
    return charge


def test_consistency(Q, X0, X1, Sigma, x, y, z, mask_indices, device='cuda'):
    result1 = gauss_conv_line_3d_orig(Q, X0, X1, Sigma, x, y, z, device)
    
    # Create boolean mask from indices for comparison
    mask = torch.zeros_like(x, dtype=torch.bool, device=device)
    mask[mask_indices[:, 1], mask_indices[:, 2], mask_indices[:, 3]] = True
    
    result2 = gauss_conv_line_3d_mask(Q, X0, X1, Sigma, x, y, z, mask_indices, device)
...
def main():
    ...
    # Create sparse indices for each sample
    num_ones = int(0.01 * (ndim * ndim * ndim))  # 1% of total elements per sample
    batch_indices = torch.arange(nevent, device=device).repeat_interleave(num_ones)
    flat_indices = torch.randint(0, ndim*ndim*ndim, (nevent * num_ones,), device=device)
    
    # Convert flat indices to 3D coordinates
    z_indices = flat_indices // (ndim * ndim)
    y_indices = (flat_indices % (ndim * ndim)) // ndim
    x_indices = flat_indices % ndim
    
    # Combine into single tensor of indices
    mask_indices = torch.stack([batch_indices, x_indices, y_indices, z_indices], dim=1)
    ...
    for _ in range(3):
        with torch.no_grad():
            ...
            _ = gauss_conv_line_3d_mask(Q, X0, X1, Sigma, x, y, z, mask_indices, device)
